...
class App(tk.Tk):
    def __init__(self):
        ...
        # ====== Editor (CÓDIGO DE ENTRADA) ======
        left = ttk.LabelFrame(self, text="Código de entrada")
        ...
        self.line_nums.config(
            yscrollcommand=lambda *a: self._on_yscroll(*a, widget=self.line_nums)
        )
        ...
        btns.pack(fill=tk.X)
        ttk.Button(
            btns, text="Analizar (2 pasadas)", command=self.analizar
        ).pack(side=tk.LEFT, padx=4, pady=4)
        ttk.Button(btns, text="Limpiar tablas", command=self.limpiar).pack(
            side=tk.LEFT, padx=4, pady=4
        )
        ttk.Button(
            btns, text="Exportar triplos (TXT)", command=self.exportar_triplos_txt
        ).pack(side=tk.LEFT, padx=4, pady=4)
        ...
        self.notebook.add(frame_opt, text="Código optimizado")
        self.txt_opt = tk.Text(
            frame_opt, wrap="none", font=("Consolas", 11), state="disabled"
        )
        self.txt_opt.pack(fill=tk.BOTH, expand=True, padx=6, pady=6)
        xscroll_opt = ttk.Scrollbar(frame_opt, orient="horizontal", command=self.txt_opt.xview)
        xscroll_opt.pack(fill=tk.X, padx=6)
        self.txt_opt.config(xscrollcommand=xscroll_opt.set)

        # ----- Símbolos -----
        frame_sym = ttk.Frame(self.notebook)
        self.notebook.add(frame_sym, text="Símbolos")
        self.tree_sym = ttk.Treeview(
            frame_sym, columns=("lex", "tipo"), show="headings", height=20
        )
    ...
    def analizar(self):
        try:
            ...
            # 1) Código original del editor (CÓDIGO DE ENTRADA)
            texto_original = self.txt.get("1.0", tk.END)

            # 2) OPTIMIZACIÓN: generar código optimizado ANTES DE TODO
            codigo_opt = optimizar_codigo_fuente(texto_original)

            # 3) Mostrar SIEMPRE el código optimizado en su pestaña
            self.txt_opt.config(state="normal")
            self.txt_opt.delete("1.0", tk.END)
            self.txt_opt.insert("1.0", codigo_opt)
            self.txt_opt.config(state="disabled")

            # 4) Analizar SIEMPRE el código optimizado
            texto_a_analizar = codigo_opt

            tabla, errores, triplos = analizar_y_triplos(texto_a_analizar)
            ...
            # Ir directo a Triplos
            self.notebook.select(3)
            messagebox.showinfo(
                "Éxito",
                "Optimización aplicada.\n"
                "El análisis (símbolos, errores y triplos) se hizo sobre "
                "el código optimizado.",
            )
        except Exception as ex:
            messagebox.showerror(
                "Error", f"Ocurrió un error durante el análisis:\n{ex}"
            )

    def exportar_triplos_txt(self):
        if not self._last_triplos:
            messagebox.showwarning(
                "Sin datos", "Primero ejecuta 'Analizar' para generar triplos."
            )
            return
        ...
        try:
            ...
            def row(vals):
                ...
                for i, c in enumerate(vals):
                    out.append(
                        " "
                        + (c.rjust(widths[i]) if i == 0 else c.ljust(widths[i]))
                        + " "
                    )
                return "|" + "|".join(out) + "|\n"
